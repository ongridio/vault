...
source: https://docs.docker.com/engine/userguide/networking/#the-default-bridge-network
kind: external
domain: kernel
original_date: 2026-03-17
fetched_at: 2026-05-16
bookmark_title: Docker container networking | Docker Documentation
tags: [external, kernel]
---

> [!info] 外部文章 · 自动导入
> 来源：[docs.docker.com](https://docs.docker.com/engine/userguide/networking/#the-default-bridge-network)
> 原始日期：2026-03-17
> 抓取日期：2026-05-16
...
container without the `--network`

option, it is connected to the default
bridge.
...
When you create or run a container using `docker create`
...
flag to make a port available outside the host,
...
option is provided, Docker automatically selects a subnet from predefined "default address pools".
These pools can be configured in `/etc/docker/daemon.json`

. Docker's built-in default is equivalent to:
...
: The subnet that can be allocated from.`size`

: The prefix length used for each allocated subnet.

When an IPv6 subnet is required and there are no IPv6 addresses in `default-address-pools`

, Docker allocates
subnets from a Unique Local Address (ULA) prefix. To use specific IPv6 subnets instead, add them to your
`default-address-pools`

. See Dynamic IPv6 subnet allocation
for more information.

Docker attempts to avoid address prefixes already in use on the host. However, you may need to customize
`default-address-pools`

to prevent routing conflicts in some network environments.
...
.
Docker will allocate subnets `172.17.0.0/24`

, `172.17.1.0/24`

, and so on, up to `172.17.255.0/24`
...
NoteSupport for unspecified addresses in

`--subnet`

was introduced in Docker 29.0.0. If Docker is downgraded to an older version, networks created in this way will become unusable. They can be removed and re-created, or will function again if the daemon is restored to 29.0.0 or later.
...
127.0.0.1, then running the `redis-cli`
