---
title: GitHub - jpetazzo/pipework: Software-Defined Networking tools for LXC (LinuX Containers)
source: https://github.com/jpetazzo/pipework#let-the-docker-host-communicate-over-macvlan-interfaces
kind: external
domain: network
author: Jpetazzo
original_date: 2013-06-13
fetched_at: 2026-05-16
bookmark_title: jpetazzo/pipework: Software-Defined Networking tools for LXC (LinuX Containers)
tags: [external, network]
---

> [!info] 外部文章 · 自动导入
> 来源：[github.com](https://github.com/jpetazzo/pipework#let-the-docker-host-communicate-over-macvlan-interfaces)
> 作者：Jpetazzo
> 原始日期：2013-06-13
> 抓取日期：2026-05-16
...
(to disable the STP protocol and prevent the switch from disabling ports)`brctl setfd br1 2`

(to reduce the time taken by the`br1`

interface to go from*blocking*to*forwarding*state)`brctl setmaxage br1 0`
...
anymore. You might want to use the `virtio-net`
...
virtualize the interface, you can use the `--direct-phys`
...
) must be installed on your Docker*host*(you don't have to install it in your containers, but it must be present on the host), unless you specify`dhcp`

as the client, in which case the Docker`busybox`
...
- dhclient: use DHCP client
`dhclient-f`

- udhcpc: use DHCP client
`udhcpc-f`
...
- dhcp: see the next section DHCP Options
- dhclient: pidfiles in
`/var/run/dhclient.$GUESTNAME.pid`

- udhcpc: pidfiles in
`/var/run/udhcpc.$GUESTNAME.pid`

- dhcpcd: not yet supported

`$GUESTNAME`

is the name or ID of the guest as you passed it to pipework on instantiation.
...
All you have to do is set the interface to be `route`

, followed by the container ID or name, followed by the route command.
...
Everything after the container ID (or name) will be run as an argument to `ip route`

inside the container's namespace. Use the iproute2 man page.

If you want to add/delete/replace IP rules in the container, you can do the same thing with `ip rule`
...
Specify the interface to be `rule`

, followed by the container ID or name, followed by the rule command.
...
If you want to use `tc`

from within the container namespace, you can do so with the command
`pipework tc $CONTAINERID <tc_args>`

.

Example, to simulate 30% packet loss on `eth0`
...
is installed, it will be used to send a gratuitous ARP reply
