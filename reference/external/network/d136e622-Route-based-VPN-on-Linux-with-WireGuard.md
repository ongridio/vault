...
source: https://vincent.bernat.ch/en/blog/2018-route-based-vpn-wireguard
kind: external
domain: network
...
> Source: [vincent.bernat.ch](https://vincent.bernat.ch/en/blog/2018-route-based-vpn-wireguard)
> Author: Vincent Bernat
> License: cc-by-sa
> Fetched: 2026-05-18
...
In a [previous article](/en/blog/2017-route-based-vpn "Route-based IPsec VPN on Linux with strongSwan"), I described an implementation of redundant
site-to-site VPNs using IPsec (with [strongSwan](https://strongswan.org/ "strongSwan: the OpenSource IPsec-based VPN Solution") as an IKE daemon) and BGP
(with [BIRD](https://bird.network.cz/ "The BIRD Internet Routing Daemon")) to achieve this: 🦑
...
As an alternative to IPsec, [WireGuard](https://www.wireguard.com/ "WireGuard: fast, modern, secure VPN tunnel") is an extremely simple (less than
5,000 lines of code) yet fast and modern VPN that utilizes state-of-the-art and
opinionated cryptography ([Curve25519](http://cr.yp.to/ecdh.html "Curve25519: high-speed ellipitic-curve cryptography"), [ChaCha20](http://cr.yp.to/chacha.html "The ChaCha family of stream ciphers"), [Poly1305](http://cr.yp.to/mac.html "Poly1305-AES: a state-of-the-art message-authentication code")) and whose
protocol, based on [Noise](http://www.noiseprotocol.org/ "Noise Protocol Framework"), has been [formally verified](https://www.wireguard.com/formal-verification/ "WireGuard: formal verification using Tamarin"). ~~It is
...
Then, for each remote VPN, we create a short configuration file:[1](#sidenote-simplicity)

1

Compared to IPsec, the cryptography is not configurable and you
...
as the routing is dynamic. The `AllowedIPs` directive tells WireGuard to accept and send
any traffic.
...
Like VTI interfaces, *WireGuard* tunnel interfaces are [namespace-aware](https://www.wireguard.com/netns/ "Routing & Network Namespace Integration"): once
...
The remote end uses `2001:db8:ff::1/127`. Once everything is set up, from one
...
*BIRD* configuration is unmodified compared to our [previous setup](/en/blog/2017-route-based-vpn "Route-based IPsec VPN on Linux with strongSwan") and the BGP sessions should establish quickly:
...
Remote routes are learned over the different tunnel interfaces:
...
You can find all the configuration files for this example on [GitHub](https://github.com/vincentbernat/network-lab/tree/master/lab-routed-vpn-wireguard "Configuration files for route-based VPNs with WireGuard").

Update (2018-11)

It is also possible to transport IPv4 on top of IPv6
*WireGuard* tunnels. The lab has been [updated](https://github.com/vincentbernat/network-lab/commit/d9c5f48f5bc092f1f1d2ba5fe307c40eca80f7c3 "routed-vpn-wireguard: also enable IPv4 over IPv6") to support such a
scenario.
