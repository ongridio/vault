---
title: Search
source: https://www.linuxjournal.com/article/7356
kind: external
domain: network
author: Kevin Kaichuan He
original_date: 2005-01-05
fetched_at: 2026-05-16
bookmark_title: Kernel Korner - Why and How to Use Netlink Socket | Linux Journal
tags: [external, network]
---

> [!info] 外部文章 · 自动导入
> 来源：[www.linuxjournal.com](https://www.linuxjournal.com/article/7356)
...
indefinitely. Use `skb = skb_recv_datagram(nl_sk)`
where `nl_sk` is the netlink socket returned by
...
`skb->data` points to the netlink message to be sent and
`pid` is the receiving application's pid, assuming NLPID Formula 1 is used.
`nonblock` indicates whether the API should block when the receiving buffer
...
`allocation` is the kernel memory allocation type. Typically, GFP_ATOMIC
