...
source: https://wiki.archlinux.org/title/Disk_encryption
kind: external
domain: storage
author: ArchWiki
license: cc-by-sa
...
> Source: [wiki.archlinux.org](https://wiki.archlinux.org/title/Disk_encryption)
> Author: ArchWiki
> License: cc-by-sa
> Fetched: 2026-05-18
...
This article discusses [data-at-rest](https://en.wikipedia.org/wiki/Data_at_rest "wikipedia:Data at rest") [encryption](https://en.wikipedia.org/wiki/Disk_encryption "wikipedia:Disk encryption") software, which on-the-fly encrypts / decrypts data written to / read from a [block device](/title/Block_device "Block device"), [disk partition](/title/Disk_partition "Disk partition") or directory. Examples for block devices are hard drives, flash drives and DVDs.
...
Data-at-rest encryption does not protect your data from all threats.
...
:   - swap partitions
    - `/tmp` (temporary files created by user applications)
      - (potential remedies: avoid such applications; mount `/tmp` inside a [ramdisk](/title/Ramdisk "Ramdisk"))
    - `/var` (log files and databases and such; for example, [locate](/title/Locate "Locate") stores an index of all file names in `/var/lib/plocate/plocate.db`)
...
Which encryption setup is appropriate for you will depend on your goals (please read [#Why use encryption?](#Why_use_encryption?) above) and system parameters.
...
Then you can go on to make the required technical choices (see [#Available methods](#Available_methods) and [#How the encryption works](#How_the_encryption_works) below), regarding:
...
Once you have decided which kind of disk erasure you want to perform, refer to the [Securely wipe disk](/title/Securely_wipe_disk "Securely wipe disk") article for technical instructions.
...
Available solutions in this category include [eCryptfs](/title/ECryptfs "ECryptfs") and [EncFS](/title/EncFS "EncFS"), or one of the cloud-ready options below.
...
Note that some cloud-storage services offer zero-knowledge encryption directly through their own [client applications](/title/List_of_applications/Internet#Cloud_synchronization_clients "List of applications/Internet").
...
2. No for ZFS. For other filesystem-native encryption mount restrictions may apply.
...
Fill in blanks. Add sources to checkmarks / crosses. What is
...
[Talk:Data-at-rest encryption](https://wiki.archlinux.org/title/Talk:Data-at-rest_encryption)
...
Example 1
:   Simple user data encryption (internal hard drive) using a virtual folder called `~/Private` in the user's home directory encrypted with [EncFS](/title/EncFS "EncFS")

    - encrypted versions of the files stored on-disk in `~/.Private`
    - unlocked on demand with dedicated passphrase

Example 2
:   Partial system encryption with each user's home directory encrypted with [ECryptfs](/title/ECryptfs "ECryptfs")

    - unlocked on respective user login, using login passphrase
    - `swap` and `/tmp` partitions encrypted with [Dm-crypt with LUKS](/title/Dm-crypt_with_LUKS "Dm-crypt with LUKS"), using an automatically generated per-session throwaway key
    - indexing/caching of contents of `/home` by *slocate* (and similar applications) disabled.

Example 3
:   System encryption - whole hard drive except `/boot` partition (however, `/boot` can be encrypted with [GRUB](/title/Dm-crypt/Encrypting_an_entire_system#Encrypted_boot_partition_(GRUB) "Dm-crypt/Encrypting an entire system")) encrypted with [Dm-crypt with LUKS](/title/Dm-crypt_with_LUKS "Dm-crypt with LUKS")
    ...
    - Maybe encryption spanning multiple drives or partition layout flexibility with [LUKS on LVM](/title/Dm-crypt/Encrypting_an_entire_system#LUKS_on_LVM "Dm-crypt/Encrypting an entire system")

Example 4
:   Hidden/plain system encryption - whole hard drive encrypted with [plain dm-crypt](/title/Dm-crypt "Dm-crypt")
...
| --- | --- | --- | --- |
| [AES](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard "wikipedia:Advanced Encryption Standard") | 128 bits | 128, 192 or 256 bits | Approved by the NSA for protecting "TOP SECRET" classified US-government information when used with a key size of 256 bits (CNSA 2.0). [128- and 192-bit keys remain safe](https://eprint.iacr.org/2019/1492.pdf) against [non-quantum adversaries](https://www.fierceelectronics.com/electronics/aes-256-joins-quantum-resistance) (same for below). Commonly has fast hardware implementation. |
| [Blowfish](https://en.wikipedia.org/wiki/Blowfish_(cipher) "wikipedia:Blowfish (cipher)") | 64 bits | 32–448 bits | One of the first patent-free secure ciphers that became publicly available, hence very well established on Linux |
...
| [Serpent](https://en.wikipedia.org/wiki/Serpent_(cipher) "wikipedia:Serpent (cipher)") | 128 bits | 128, 192 or 256 bits | Considered the most secure of the five AES-competition finalists[[4]](https://csrc.nist.gov/archive/aes/round2/r2report.pdf)[[5]](https://www.cl.cam.ac.uk/~rja14/Papers/serpentcase.pdf)[[6]](https://www.cl.cam.ac.uk/~rja14/Papers/serpent.pdf). |
...
Stream ciphers work in streams and do not need such thing as a "mode of operation". Nevertheless, properties required for disk encryption (e.g. "wide-block") mean that layers need to be added on top of them. As far as disk encryption on Linux is concerned, the main example is XChaCha12 in the [Adiantum](https://lwn.net/Articles/776721/) configuration, `xchacha12,aes-adiantum`, intended for low-end systems where the hardware has no AES acceleration. Adiantum is fast on such hardware. It also guarantees authenticity of the encrypted data.

### Plausible deniability

See [Wikipedia:Plausible deniability](https://en.wikipedia.org/wiki/Plausible_deniability "wikipedia:Plausible deniability") and [Wikipedia:Deniable Encryption](https://en.wikipedia.org/wiki/Deniable_Encryption "wikipedia:Deniable Encryption").

## Backup for disk encryption scenarios

Make a [backup](/title/Backup "Backup") of the user data to protect against data loss. In general the backup of your encrypted data should be encrypted as well.

### Block device encryption

There are multiple options; you can back up the disk block device where the encryption container resides as an image, e.g. `/dev/sdx`, or you can back up the filesystem inside of the encrypted container, e.g. `/dev/mapper/dm_name`, or you can back up the files, e.g. with [rsync](/title/Rsync "Rsync"). The following sections list the advantages and disadvantages of each option.
...
- only as large as the used space on the filesystem, see e.g. [partclone](/title/Partclone "Partclone")
...
If using LUKS it is possible to make a [backup of the LUKS headers](/title/Dm-crypt/Device_encryption#Backup_and_restore "Dm-crypt/Device encryption"): it can make sense to periodically check and synchronize those backups, especially if passphrases have been revoked.
