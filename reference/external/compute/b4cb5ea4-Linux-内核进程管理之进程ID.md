...
source: http://www.cnblogs.com/hazir/p/linux_kernel_pid.htm
kind: external
domain: compute
author: Hazir
original_date: 2013-10-03
fetched_at: 2026-05-16
bookmark_title: Linux 内核进程管理之进程ID - hazir - 博客园
tags: [external, compute]
---

> [!info] 外部文章 · 自动导入
> 来源：[www.cnblogs.com](http://www.cnblogs.com/hazir/p/linux_kernel_pid.htm)
> 作者：Hazir
> 原始日期：2013-10-03
> 抓取日期：2026-05-16
...
Linux 内核使用 `task_struct`

数据结构来关联所有与进程有关的数据和结构，Linux 内核所有涉及到进程和程序的所有算法都是围绕该数据结构建立的，是内核中最重要的数据结构之一。该数据结构在内核文件 `include/linux/sched.h`

中定义，在Linux 3.8 的内核中，该数据结构足足有 380 行之多，在这里我不可能逐项去描述其表示的含义，本篇文章只关注该数据结构如何来组织和管理进程ID的。
...
- 周徐达师弟的PPT（让我受益匪浅的一次讨论，周由浅入深告诉我们该数据结构是如何设计出来的，本文主思路就是按照该PPT，在此
**特别感谢**！）
